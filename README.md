...
## Сценарий использования GTD Clarify
...
За счет этого проект представляет собой не абстрактный “чат с API”, а агент с конкретной доменной логикой и понятным сценарием использования.
...
- Агент не запрашивает у пользователя внутренние идентификаторы TickTick, если может определить их самостоятельно.
- Inbox нормализован через отдельный alias, что снижает количество ошибок в проектных ссылках.
- Tool loop поддерживает работу с несколькими моделями и fallback-логику при ошибках OpenRouter.
- Представление задач отделено от сырых API payload, благодаря чему модель получает более чистый и стабильный контекст.
- Агент спроектирован вокруг конкретного GTD-сценария, а не как универсальная оболочка для чата.
